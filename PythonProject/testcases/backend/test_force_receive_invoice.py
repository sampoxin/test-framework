...
class TestForceReceiveInvoice:
    ...
    @pytest.mark.p0
    @pytest.mark.order(1)
    def test_01_query_unsettled_match(self, admin_client, context):
        ...
        with allure.step("步骤3：搜索指定供应商的未结算收/退货单"):
            ...
            result_json = admin_client.send_and_validate("POST", "/api/v1/admin/srm/force-match/page-unsettled",
                                json={"supplierId":supplier_id,"pageNum":1,"pageSize":20,"orderNos":self.receipt_nos,"returnNos":self.return_nos})
            assert result_json["data"]["total"] > 0
            context["receipts"] = [ item["receiptOrReturnNo"] for item in result_json["data"]["records"] if item["docType"] == "RECEIPT"]

    @allure.story("生成发票")
    @pytest.mark.p0
    @pytest.mark.order(2)
    def test_02_generate_invoice(self, admin_client, context):
        supplier_id = context["supplier_id"]
        receipts = context["receipts"]

        with allure.step("步骤1：搜索指定供应商的未结算收/退货单"):
            result_json = admin_client.send_and_validate("POST", "/api/v1/admin/srm/force-match/page-unsettled",
                                json={"supplierId":supplier_id,"pageNum":1,"pageSize":12,"receiptNos":receipts,"returnNos":self.return_nos})
            assert result_json["data"]["total"] > 0
        ...
        with allure.step("步骤2：读取未结算收/退货单详情"):
            result_json = admin_client.send_and_validate("POST", "/api/v1/admin/srm/force-match/load-receipt-items",
                                json={"supplierId":supplier_id,"receiptNos":receipts,"returnNos":self.return_nos})
            receipt_items = convert_receipt_data(result_json)

        with allure.step("步骤3：生成发票"):
            self._generate_invoice( admin_client, context, self.invoice_nos, supplier_id, receipt_or_return_nos, receipt_items)


    def _generate_invoice(self, admin_client, context, invoice_nos, supplier_id, receipt_or_return_nos, receipt_items):
            result_json = admin_client.send_and_validate("POST", "/api/v1/admin/srm/force-match/submit",
                                json={"supplierId":supplier_id,"invoiceNos":invoice_nos,"receiptOrReturnNos":receipt_or_return_nos,"items":receipt_items})
    ...
    @pytest.mark.p0
    @pytest.mark.order(3)
    def test_03_expense_sheet(self, admin_client, context):
    ...
    @pytest.mark.p0
    @pytest.mark.order(4)
    def test_04_approval_pass(self, admin_client, context):
        match_id = context.get("match_ids", [])[0]
        supplier_id = context.get("supplier_id", "")
    ...
    @pytest.mark.p0
    @pytest.mark.order(5)
    def test_05_push_settlement(self, admin_client, context, file_helper):
